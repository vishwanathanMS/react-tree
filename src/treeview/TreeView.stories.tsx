...
const meta: Meta<typeof TreeView> = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'A feature-rich, high-performance React TreeView component supporting virtualization (100k+ nodes), checkboxes, multi-selection, drag-and-drop reordering, inline editing, lazy loading, custom node rendering, and custom field mapping.',
      },
    },
  },
  argTypes: {
...
// 17. Virtualized 100,000+ Nodes (All Nodes Checked)
export const Virtualized100kAllChecked = () => {
  const { data100k, allNodeIds, allParentIds } = useMemo(() => {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      for (let j = 1; j <= 10; j++) {
        const l2Id = `${l1Id}-L2-${j}`;
        nodeIds.push(l2Id);
        if (i === 1 && j === 1) parentIds.push(l2Id);
        ...
        for (let k = 1; k <= 10; k++) {
          const l3Id = `${l2Id}-L3-${k}`;
          nodeIds.push(l3Id);
          if (i === 1 && j === 1 && k === 1) parentIds.push(l3Id);
          ...
          for (let m = 1; m <= 10; m++) {
            const l4Id = `${l3Id}-L4-${m}`;
            nodeIds.push(l4Id);
            if (i === 1 && j === 1 && k === 1 && m === 1) parentIds.push(l4Id);
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: 500, border: '1px solid #CBD5E1', borderRadius: 12, padding: 16, background: '#FFFFFF' }}>
      <h3 style={{ margin: '0 0 4px 0', fontSize: 16 }}>☑️ 100,000+ Nodes (All 111,110 Nodes Checked)</h3>
      <p style={{ margin: '0 0 12px 0', fontSize: 12, color: '#64748B' }}>
        Virtual windowing with all 111,110 nodes checked in state. Demonstrates fast checkbox calculation and tri-state handling over 100k items!
      </p>
